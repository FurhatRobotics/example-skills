...
public class TranscribeStreamingRetryClient {
     ...
     */
    public TranscribeStreamingRetryClient(AwsCredentialsProvider creds, Region region) throws URISyntaxException {
        ...
        result.whenComplete((r, e) -> {
            if (e != null) {
                ...
                if (retryAttempt <= maxRetries && isExceptionRetriable(e)) {
                    log.info("Retriable error occurred and will be retried.");
                    log.info("Sleeping for sometime before retrying...");
                    try {
                        Thread.sleep(sleepTime);
                    } catch (InterruptedException e1) {
                        log.info("Unable to sleep. Failed with exception: ", e);
                        e1.printStackTrace();
                    }
                    log.info("Making retry attempt: " + (retryAttempt + 1));
